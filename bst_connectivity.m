...
%==================================  PLI edited area start  =======================================
                % R(:,:,iBand) = (cA*cB' + sA*sB' + 1i * (sA*cB' - cA*sB')) ./ size(cA,2);

                [m,n] = size(DataAband');
                d_angle = zeros(m,1);
                angles = angle (hilbert(DataAband'));
                for ch1 = 1:n
                    for ch2 = 1:n
                        d_angle = wrapToPi(angles(:,ch1) - angles(:,ch2));
                        R(ch1,ch2,iBand) = abs ( mean ( sign ( d_angle ) ) );
                    end
                end
%==================================  PLI edited area end  =======================================
            end
...
%===================  DBWPLI edited area start  ==========================
    ...
    for n = 1:nAvg
        if isempty(Radd)
            ...
            R2add = Rall(:,:,:,n).*Rall(:,:,:,n)
            Raddabs = abs(Rall(:,:,:,n))
            R2addabs = abs(Rall(:,:,:,n).*Rall(:,:,:,n))
        else
            Radd = Radd + Rall(:,:,:,n)
            R2add = R2add + Rall(:,:,:,n).*Rall(:,:,:,n)
            Raddabs = Raddabs + abs(Rall(:,:,:,n))
            R2addabs = R2addabs + abs(Rall(:,:,:,n).*Rall(:,:,:,n))
        end
    end

    dbWPLI = (Radd.*Radd - R2add) ./ (Raddabs.*Raddabs - R2addabs)
    OutputFiles{1} = SaveFile(dbWPLI, OPTIONS.iOutputStudy, [], sInputA, sInputB, Comment, nAvg, OPTIONS, FreqBands);
%===================  DBWPLI edited area end  ==========================
end
